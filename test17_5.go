...
func main() {	//	通道不带缓冲，表示同步的，只能向通道 c 发送一个数据，只要这个数据没被接收然后所有的发送就被阻塞
	s := []int{7, 2, 8, -9, 4, 0}
	c := make(chan int)
	fmt.Println("go [0,3]")
	...
	//	a b c d 和 main 一起争夺cpu的，他们的执行顺序完成无序，甚至里面不同的语句都相互穿插
	//	但无缓冲的等待是同步的，所以接下来 a b c d 都会执行，一直执行到 c <- sum后，开始同步阻塞
	...
	//	因为 a b c d 都在管道门口等着，这里度一个 a b c d 就继续一个，这个结果的顺序可能是 a b c d
	aa := <- c
	bb := <- c
	fmt.Println(aa)
	fmt.Println(bb)
